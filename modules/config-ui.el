...
(defun split-window-below-cycle (&optional size window-to-split)
...
(defun split-window-right-cycle (&optional size window-to-split)
  (interactive `(,(when current-prefix-arg
                    (prefix-numeric-value current-prefix-arg))
                 ,(selected-window)))
...
(global-set-key (kbd "C-x 2") #'split-window-below-cycle)
(global-set-key (kbd "C-3") #'split-window-right-cycle)
(global-set-key (kbd "C-x 3") #'split-window-right-cycle)
